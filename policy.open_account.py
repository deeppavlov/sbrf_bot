    ...
    if params['slot'] == 'currency':
        ...
        if state["intent"][0] == "RESERVE_ACCOUNT":
            ...
            return f"Какая информация вас интересует: сроки, тарифы, комплект документов, " \
                   f"удаленное обслуживание или процедура открытия?", confidence, state
        else:
...
def get():
    return [
        ...
        (lambda s: len(s["intent"]) >=1 and s["intent"][0] == "OPEN_ACCOUNT" and not s["properties"],
         lambda s: ask_slot(s, {'slot': 'properties'})),
        (lambda s: len(s["intent"]) >= 1 and s["intent"][0] == "RESERVE_ACCOUNT" and not s["properties"],
         lambda s: ask_slot(s, {'slot': 'properties'})),
        (lambda s: len(s["intent"]) >= 1 and s["intent"][0] == "OPEN_ACCOUNT" and s["properties"],
         provide_info_for_openning),
        (lambda s: len(s["intent"]) >= 1 and s["intent"][0] == "RESERVE_ACCOUNT" and s["properties"],
         provide_info_for_reservation),
